...
const AddList3 = () => {
  ...
  const [selectedImage, setSelectedImage] = useState({ id: '', imgUrl: '', rating: '', name: '' });
  ...
  const loadData = async () => {
    try {
      const storedImageList = await AsyncStorage.getItem('ads_app');
      if (storedImageList) {
  ...
  const saveData = async (updatedList) => {
    try {
      ...
      await AsyncStorage.setItem('ads_app', jsonList);
    } catch (error) {
  ...
  const addImage = async (image) => {
    ...
    setSelectedImage({ id: '', imgUrl: '', rating: '', name: '' })
  };
  ...
  return (
    <View style={{ flex: 1, padding: 20 }}>
      <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 10 }}>Danh sách ứng dụng :</Text>
      <ScrollView style={{ flex: 1 }}>
        <FlatList
          data={imageList}
          renderItem={renderItem}
          keyExtractor={(item) => item.id.toString()}
          ListEmptyComponent={<Text>Không có ứng dùng nào</Text>}
      ...
      <Modal isVisible={isModalVisible} onBackdropPress={toggleModal}>
        <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10 }}>
          <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 10 }}>Thông tin ứng dụng</Text>
          <Text>URL ảnh:</Text>
          <TextInput
          ...
          <Text>Rating:</Text>
          <TextInput
            value={selectedImage.rating}
            onChangeText={(text) => setSelectedImage({ ...selectedImage, rating: text })}
            style={{ borderWidth: 1, borderColor: 'gray', marginBottom: 10, borderRadius: 5, padding: 10 }}
          />
          ...
          <TextInput
            ...
            onChangeText={(text) => setSelectedImage({ ...selectedImage, name: text })}
            style={{ borderWidth: 1, borderColor: 'gray', marginBottom: 10, borderRadius: 5, padding: 10 }}
          />
